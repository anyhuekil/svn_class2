...
th {
	...
	background: -moz-linear-gradient(top, #feffff 0%, #ddf1f9 40%, #ddf1f9 42%, #a0d8ef
		100%); /* FF3.6-15 */
	background: -webkit-linear-gradient(top, #feffff 0%, #ddf1f9 40%, #ddf1f9 42%,
		#a0d8ef 100%); /* Chrome10-25,Safari5.1-6 */
	background: linear-gradient(to bottom, #feffff 0%, #ddf1f9 40%, #ddf1f9 42%, #a0d8ef
		100%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
	filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#feffff',
		endColorstr='#a0d8ef', GradientType=0); /* IE6-9 */
	margin-bottom: 0;
}

input[name=operator], input[name=score] {
	border: 0;
	font-weight: bold;
	font-size: 1.1em;
	text-align: center;
...
tr, input[name=operator], input[name=score] {
	...
	<form action="" onsubmit="return scoreResult()">
		<table>
			<tr>
				<th colspan="2">입력</th>
				<th>결과입력</th>
				<td rowspan="4"><input type="submit" value="채점"></td>
				<th>채점결과</th>
			</tr>
			<% for(int i=0 ; i<3 ; i++) {%>
			<tr>
				<td class="no"><%=(i+1) %></td>
				<td class="cal"><input type="text" name="firstNum"
					value="<%=(int)(Math.random()*100)%>" size="1"> <input
					...
					size="1"> <input type="text" name="secondNum"
					value="<%=(int)(Math.random()*100)%>" size="1"></td>
				<td><input type="text" name="result" size="1"></td>
				<td class="result"><input type="text" name="score" size="10"
					value="<%
 						if(firstNum!=null){
 							switch(operator[i]){
